...
export function HomeCatalog({ products }: Props) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      const matchesSegment = segment === "all" || info.segment === segment;
      const matchesQuery = !normalizedQuery || normalizeSearch(getSearchText(product)).includes(normalizedQuery);
  ...
  function scrollRail(direction: -1 | 1) {
    railRef.current?.scrollBy({ left: direction * 350, behavior: "smooth" });
  }
    ...
    <section className={styles.catalog} id="sistemas" aria-labelledby="catalog-title">
      <div className="shell">
        <div className={styles.heading}>
          <div>
            <p className={styles.kicker}>Encontre sem perder tempo</p>
            <h2 id="catalog-title">O sistema certo para o seu segmento.</h2>
          </div>
        ...
        <label className={styles.search}>
          ...
          <input
            ...
            placeholder="Pesquisar segmento, necessidade ou app"
            aria-label="Pesquisar segmento, necessidade ou aplicativo"
          />
        ...
        <div className={styles.filters} aria-label="Filtrar aplicativos por segmento">
          {segmentFilters.map((filter) => (
            <button
              type="button"
              key={filter.id}
              className={segment === filter.id ? styles.filterActive : undefined}
              onClick={() => selectSegment(filter.id)}
              aria-pressed={segment === filter.id}
            >
        ...
        <div className={styles.resultBar}>
          <strong>{filteredProducts.length === 1 ? "1 aplicativo encontrado" : `${filteredProducts.length} aplicativos encontrados`}</strong>
          <span>Arraste para o lado ou use as setas</span>
        </div>

        {filteredProducts.length ? (
          <div className={styles.carouselShell}>
            <button className={`${styles.railButton} ${styles.railButtonLeft}`} type="button" onClick={() => scrollRail(-1)} aria-label="Ver aplicativos anteriores">‹</button>
            <div className={styles.rail} ref={railRef}>
              {filteredProducts.map((product) => <ProductCard key={product.slug} product={product} />)}
            </div>
            <button className={`${styles.railButton} ${styles.railButtonRight}`} type="button" onClick={() => scrollRail(1)} aria-label="Ver próximos aplicativos">›</button>
          </div>
        ) : (
          <div className={styles.empty}>
            <ProductIcon slug="pandora" size={30} />
            <h3>Nenhum aplicativo encontrado</h3>
            <p>Tente pesquisar por palavras como oficina, pet shop, orçamento, imóveis, eventos ou biblioteca.</p>
            <button type="button" onClick={() => { setQuery(""); selectSegment("all"); }}>Mostrar todos</button>
          </div>
        )}
      </div>
    </section>
...
function ProductCard({ product }: { product: Product }) {
  ...
  const mediaStyle = {
    ...
    "--product-soft": product.colorSoft,
    "--media-image": `url("${media.gallery[galleryIndex]}")`,
  } as CSSProperties;
  ...
  return (
    <article className={styles.card} style={mediaStyle}>
      <div
        ...
        onMouseEnter={() => void startVideo()}
        onMouseLeave={() => stopVideo(true)}
        onClick={toggleVideo}
        role="button"
        tabIndex={0}
      ...
      >
        {!videoUnavailable ? (
          <video
            ref={videoRef}
            className={videoActive ? styles.videoActive : undefined}
            src={media.video}
            poster={media.gallery[galleryIndex]}
            muted
            loop
            playsInline
            preload="metadata"
            onError={() => { setVideoUnavailable(true); setVideoActive(false); }}
            aria-hidden="true"
          />
        ) : null}
      ...
      <div className={styles.cardBody}>
        <div className={styles.cardIdentity}>
          <span><ProductIcon slug={product.slug} size={21} /></span>
          <div>
            <h3>{product.name}</h3>
        ...
        <div className={styles.cardActions}>
          <Link className={styles.learnMore} href={`/aplicativos/${product.slug}`}>Saiba mais</Link>
          <Link className={styles.subscribe} href={`/assinar/${product.slug}`}>Assinar agora</Link>
        </div>
      </div>
    </article>
  );
