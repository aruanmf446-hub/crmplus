...
import trust from "./CommercialTrust.module.css";
...
const signatures: Record<string, SignatureConfig> = {
  atlas: {
    ...
    metric: "08",
    metricLabel: "atendimentos no exemplo",
  ...
  poseidon: {
    ...
    metric: "R$ 184 mil",
    metricLabel: "pipeline ilustrativo",
  ...
  hercules: {
    ...
    metric: "82%",
    metricLabel: "progresso ilustrativo",
    labels: ["Conforme", "Não conforme", "Evidência", "Correção"],
    details: ["18 itens verificados", "2 desvios abertos", "6 imagens", "1 prazo próximo"],
  },
  zeus: {
    kind: "fleet",
    ...
    metric: "14",
    metricLabel: "veículos no exemplo",
  ...
  alexandria: {
    ...
    metric: "1.248",
    metricLabel: "obras no exemplo",
    labels: ["Acervo", "Reservas", "Emprestados", "Atrasados"],
    details: ["Dom Casmurro · 1 disponível", "Engenharia · reservado", "História do Pará · emprestado", "4 devoluções previstas"],
  },
  olympus: {
    kind: "property",
    ...
    metric: "38",
    metricLabel: "imóveis no exemplo",
    labels: ["Captação", "Disponível", "Visita", "Proposta"],
    details: ["Apartamento · 3 quartos", "Galpão · 1.400 m²", "Casa · visita agendada", "2 propostas abertas"],
  },
  argus: {
    kind: "assets",
  ...
  pegasus: {
    ...
    metric: "09",
    metricLabel: "pets no exemplo",
    labels: ["Agenda", "Chegada", "Cuidado", "Entrega"],
    details: ["Thor · atenção especial", "Luna · banho sem perfume", "2 hospedagens", "1 retirada agendada"],
  },
  titans: {
    kind: "construction",
    ...
    metric: "42%",
    metricLabel: "avanço ilustrativo",
    ...
    <section className={styles.section} data-signature={config.kind} aria-labelledby={`signature-${product.slug}`}>
      <div className="shell">
        <header className={styles.header}>
          <div>
            <span className={trust.exampleLabel}>Exemplo de interface · dados ilustrativos</span>
            <h2 id={`signature-${product.slug}`}>{config.title}</h2>
            <p>{config.description}</p>
          </div>
          <div className={styles.metric}><strong>{config.metric}</strong><span>{config.metricLabel}</span><small className={trust.metricNote}>Não representa dados reais.</small></div>
        </header>
