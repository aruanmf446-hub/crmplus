...
export function PandoraApp({ product }: { product: Product }) {
  ...
  function createSurvey() {
    const id = uid("PESQ");
    const survey: Survey = { id, name: surveyDraft.name.trim(), question: surveyDraft.question.trim(), active: true, responses: 0, createdAt: todayLabel(), link: `crmplus.store/pesquisa/${id}` };
    setSurveys((current) => [survey, ...current]); setSurveyDraft({ name: "", question: "De 0 a 10, quanto você recomendaria nossa empresa?" }); setModal(null); setActive("Pesquisas"); setToast("Pesquisa criada");
  }

  async function copySurveyLink(survey: Survey) { await copyText(survey.link); setToast("Link copiado"); }
  async function prepareMessage(survey: Survey) { await copyText(`Olá! Queremos ouvir você. Responda nossa pesquisa: ${survey.link}`); setToast("Mensagem copiada"); }
  ...
  function simulateResponse() {
    ...
    setResponseSurvey(null); setModal(null); setActive("Respostas"); setToast("Resposta adicionada à caixa de entrada");
  }
  ...
  return <AppShell product={product} nav={nav} active={active} onChange={setActive} title={active} subtitle="Feedback organizado como atendimento: classificar, tratar e aprender." action={<button className={styles.primaryButton} onClick={() => setModal("survey")}><Icon name="plus" /> Nova pesquisa</button>}>
    ...
    {active === "Pesquisas" ? <section className={styles.pageSheet}><div className={styles.pageHeading}><div><span className={styles.eyebrow}>Pesquisas</span><h2>Coletas e formulários</h2><p>Crie, pause e acompanhe suas pesquisas.</p></div><button className={styles.primaryButton} onClick={() => setModal("survey")}><Icon name="plus" /> Criar pesquisa</button></div><div className={styles.surveyRows}>{surveys.map((survey) => <article key={survey.id}><div><StatusPill status={survey.active ? "Ativa" : "Pausada"} /><h3>{survey.name}</h3><p>{survey.question}</p><small>{survey.responses} respostas · criada em {survey.createdAt}</small></div><div><button className={styles.primaryButton} onClick={() => { setResponseSurvey(survey); setModal("response"); }}>Responder</button><button className={styles.secondaryButton} onClick={() => copySurveyLink(survey)}>Copiar link</button><button className={styles.secondaryButton} onClick={() => setSurveys((current) => current.map((item) => item.id === survey.id ? { ...item, active: !item.active } : item))}>{survey.active ? "Pausar" : "Ativar"}</button><button className={styles.iconButton} onClick={() => setSurveys((current) => current.filter((item) => item.id !== survey.id))}><Icon name="trash" /></button></div></article>)}</div></section> : null}

    {active === "Distribuição" ? <section className={styles.pageSheet}><div className={styles.pageHeading}><div><span className={styles.eyebrow}>Compartilhamento</span><h2>Links e materiais</h2><p>Prepare a mensagem, copie o link ou imprima o QR Code.</p></div></div><div className={styles.distributionGrid}>{surveys.filter((survey) => survey.active).map((survey) => <article key={survey.id}><div><StatusPill status="Ativa" /><h3>{survey.name}</h3><code>{survey.link}</code></div><div className={styles.linkRows}><button onClick={() => prepareMessage(survey)}><Icon name="message" /><span><strong>WhatsApp</strong><small>Copiar mensagem</small></span><Icon name="chevron" /></button><button onClick={() => copySurveyLink(survey)}><Icon name="document" /><span><strong>Link</strong><small>Copiar endereço</small></span><Icon name="chevron" /></button><button onClick={() => { setQrSurvey(survey); setModal("qr"); }}><Icon name="image" /><span><strong>QR Code</strong><small>Visualizar para impressão</small></span><Icon name="chevron" /></button></div></article>)}</div></section> : null}
    ...
    <Modal open={modal === "qr"} title={`QR Code · ${qrSurvey?.name ?? "Pesquisa"}`} description="Pronto para impressão e compartilhamento." onClose={() => setModal(null)}><div className={styles.qrPreview}><div className={styles.fakeQr}>{Array.from({ length: 81 }, (_, index) => <i key={index} className={(index * 7 + index % 4) % 3 === 0 ? styles.qrDark : ""} />)}</div><strong>{qrSurvey?.link}</strong><button className={styles.primaryButton} onClick={() => window.print()}><Icon name="print" /> Imprimir</button></div></Modal>
