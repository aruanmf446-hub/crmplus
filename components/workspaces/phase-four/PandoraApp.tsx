...
type Feedback = { id: string; customer: string; score: number; channel: string; date: string; comment: string; theme: string; status: FeedbackStatus; priority: boolean; treatment: string; owner: string; dueDate: string; customerReturn: string; history?: Array<{ text: string; date: string }> };
...
export function PandoraApp({ product }: { product: Product }) {
  ...
  return <AppShell product={product} nav={nav} active={active} onChange={changeArea} title={active} subtitle="Uma resposta por vez: analisar, agir, retornar e concluir." action={headerAction}>
    ...
    <Modal open={modal === "response"} title={`Registrar resposta · ${responseSurvey?.name ?? "Pesquisa"}`} onClose={() => setModal(null)}><Form onSubmit={saveResponse}><Field label="Cliente"><input value={responseDraft.customer} onChange={(event) => setResponseDraft((current) => ({ ...current, customer: event.target.value }))} /></Field><Field label="Nota"><input required type="number" min="0" max="10" value={responseDraft.score} onChange={(event) => setResponseDraft((current) => ({ ...current, score: event.target.value === "" ? "" : Math.min(10, Math.max(0, Number(event.target.value))) }))} placeholder="Escolha de 0 a 10" /></Field><Field label="Comentário"><textarea required value={responseDraft.comment} onChange={(event) => setResponseDraft((current) => ({ ...current, comment: event.target.value }))} /></Field><Field label="Tema"><select required value={responseDraft.theme} onChange={(event) => setResponseDraft((current) => ({ ...current, theme: event.target.value }))}><option value="">Selecione o tema</option><option>Atendimento</option><option>Tempo de resposta</option><option>Comunicação</option><option>Agilidade</option><option>Qualidade</option><option>Preço</option><option>Prazo</option></select></Field><div className={styles.modalActions}><button type="button" className={styles.secondaryButton} onClick={() => setModal(null)}>Cancelar</button><button className={styles.primaryButton}>Registrar em Novo</button></div></Form></Modal>
