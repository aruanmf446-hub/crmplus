...
export function HerculesApp({ product }: { product: Product }) {
  ...
  async function addEvidence(event: ChangeEvent<HTMLInputElement>) {
    ...
    updateItem({ evidence: [...currentItem.evidence, ...urls] }); setToast("Evidências adicionadas");
    event.target.value = "";
  }
  ...
  function createModel() {
    ...
    const model = { id: uid("MOD"), name: modelDraft.name.trim(), items: [{ label: modelDraft.item.trim(), helper: "Adicione uma orientação para este item." }] };
  ...
  return <AppShell product={product} nav={nav} active={active} onChange={setActive} title={active} subtitle="Execução guiada, evidência e tratamento de desvios no mesmo registro." action={active === "Executar inspeção" ? <button className={styles.primaryButton} disabled={readOnly} onClick={finishInspection}><Icon name="check" /> {readOnly ? "Inspeção concluída" : "Concluir inspeção"}</button> : <button className={styles.primaryButton} onClick={() => setModal(active === "Modelos" ? "model" : "schedule")}><Icon name="plus" /> {active === "Modelos" ? "Novo modelo" : "Programar"}</button>}>
    ...
    {active === "Modelos" ? <section className={styles.pageSheet}><div className={styles.pageHeading}><div><span className={styles.eyebrow}>Modelos</span><h2>Modelos de checklist</h2><p>Estruturas reutilizáveis para novas inspeções.</p></div><button className={styles.primaryButton} onClick={() => setModal("model")}><Icon name="plus" /> Novo modelo</button></div><div className={styles.templateGrid}>{models.map((model) => <article key={model.id}><div className={styles.templateIcon}><Icon name="clipboard" /></div><h3>{model.name}</h3><p>{model.items.length} item(ns) de verificação</p><div><button className={styles.secondaryButton} onClick={() => { setScheduleDraft((current) => ({ ...current, title: model.name })); setModal("schedule"); }}>Programar</button><button className={styles.iconButton} aria-label={`Remover modelo ${model.name}`} onClick={() => removeModel(model)}><Icon name="trash" /></button></div></article>)}</div></section> : null}
    ...
    <Modal open={modal === "instruction"} title={currentItem?.label ?? "Instrução"} description="Orientação do modelo de checklist." onClose={() => setModal(null)}><div className={styles.instructionBox}><Icon name="clipboard" /><p>{currentItem?.helper}</p><small>Use esta orientação como referência durante a inspeção.</small></div></Modal>
