...
export function AtlasApp({ product }: { product: Product }) {
  ...
  function createOrder() {
    const next: WorkOrder = { id: uid("OS"), ...draft, plate: draft.plate.trim().toUpperCase(), status: "Avaliação", diagnosis: "", updated: "agora", priority: false, services: [], photos: [], history: [{ text: "Ordem de serviço criada", date: todayLabel() }] };
    setOrders((current) => [next, ...current]);
    setSelectedId(next.id);
    setDraft({ client: "", phone: "", vehicle: "", plate: "", issue: "", technician: "Sem técnico" });
    setModal(null); setActive("Ordens de serviço"); setListMode("Em andamento"); setToast("Ordem de serviço criada");
  }

  function advance() {
    if (!selected) return;
    const flow: WorkStatus[] = ["Avaliação", "Aguardando aprovação", "Em serviço", "Pronto", "Entregue"];
    const index = flow.indexOf(selected.status);
    if (index < 0 || index === flow.length - 1) { setToast("Este atendimento já está concluído"); return; }
    const next = flow[index + 1];
    updateSelected({ status: next }, `Situação alterada para ${next}`);
    setToast(`OS avançou para ${next}`);
  }

  function saveDiagnosis(value: string) { updateSelected({ diagnosis: value }); }
  function addService() {
  ...
  async function addPhotos(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (files.some((file) => file.size > 700 * 1024)) {
      setToast("Escolha imagens menores, com até 700 KB cada");
      event.target.value = "";
      return;
    }
  ...
  function nextActionTab(status: WorkStatus) {
    if (status === "Avaliação") return "Diagnóstico";
    if (status === "Entregue") return "Histórico";
    return "Serviços e peças";
  }
  ...
  return <AppShell product={product} nav={nav} active={active} onChange={setActive} title={active} subtitle="Atendimento, diagnóstico, orçamento e histórico em um único registro." action={<button className={styles.primaryButton} onClick={() => setModal("new")}><Icon name="plus" /> Nova OS</button>}>
    ...
    {active === "Clientes" ? <DirectoryView title="Clientes da oficina" description="Clientes vinculados aos atendimentos registrados." rows={Array.from(new Set(orders.map((order) => order.client)))} onSelect={(row) => { const order = orders.find((item) => item.client === row); if (order) { setSelectedId(order.id); setActive("Ordens de serviço"); } }} /> : null}
    {active === "Veículos" ? <DirectoryView title="Veículos cadastrados" description="Veículos com histórico de atendimento disponível." rows={orders.map((order) => `${order.vehicle} · ${order.plate}`)} onSelect={(row) => { const order = orders.find((item) => row.includes(item.plate)); if (order) { setSelectedId(order.id); setActive("Ordens de serviço"); } }} /> : null}
