...
export function AresApp({ product }: { product: Product }) {
  ...
  function saveDraft(quote: Quote, finalize = false) {
    if (!quote.client.trim() || !quote.title.trim()) { setToast("Informe o cliente e o título da proposta"); return; }
    if (!quote.items.length || quote.items.some((item) => !item.description.trim())) { setToast("Revise os itens antes de finalizar a proposta"); return; }
    const normalized: Quote = { ...quote, updated: "agora", status: finalize ? "Pronto" : quote.status };
    setQuotes((current) => {
      const exists = current.some((item) => item.id === normalized.id);
      return exists ? current.map((item) => item.id === normalized.id ? normalized : item) : [normalized, ...current];
    });
    setSelectedId(normalized.id); setEditing(null); setActive("Orçamentos"); setToast(finalize ? "Proposta pronta para envio" : "Rascunho salvo");
  }
  ...
  return <AppShell product={product} nav={nav} active={active} onChange={(value) => { setActive(value); if (value === "Criar orçamento" && !editing) newQuote(); }} title={active} subtitle="Propostas editáveis, profissionais e acompanhadas até a decisão." action={active !== "Criar orçamento" ? <button className={styles.primaryButton} onClick={() => newQuote()}><Icon name="plus" /> Novo orçamento</button> : undefined}>
    {active === "Orçamentos" ? <div className={styles.masterDetail}>
      ...
      {selected ? <QuotePreview quote={selected} onEdit={editSelected} onShare={shareQuote} onPrint={() => window.print()} onStatus={() => setModal("status")} /> : <EmptyState icon="document" title="Nenhum orçamento selecionado" description="Escolha uma proposta na lista." />}
    </div> : null}
    ...
    {active === "Clientes" ? <section className={styles.pageSheet}><div className={styles.pageHeading}><div><span className={styles.eyebrow}>Clientes</span><h2>Clientes com propostas</h2><p>Consulte rapidamente as propostas vinculadas a cada cliente.</p></div></div><div className={styles.directoryRows}>{Array.from(new Set(quotes.map((quote) => quote.client).filter(Boolean))).map((client) => { const clientQuotes = quotes.filter((quote) => quote.client === client); return <button key={client} onClick={() => { const quote = quotes.find((item) => item.client === client); if (quote) { setSelectedId(quote.id); setActive("Orçamentos"); } }}><span className={styles.companyAvatar}>{client.slice(0, 2).toUpperCase()}</span><div><strong>{client}</strong><small>{clientQuotes.length} orçamento(s) · {currency(clientQuotes.reduce((sum, quote) => sum + quoteTotal(quote), 0))}</small></div><Icon name="chevron" /></button>; })}{!quotes.some((quote) => quote.client) ? <EmptyState icon="people" title="Nenhum cliente vinculado" description="Os clientes aparecerão após a criação das propostas." /> : null}</div></section> : null}

    {active === "Modelos" ? <section className={styles.pageSheet}><div className={styles.pageHeading}><div><span className={styles.eyebrow}>Modelos reutilizáveis</span><h2>Comece com uma estrutura pronta</h2><p>Reaproveite itens e valores como ponto de partida para novas propostas.</p></div></div><div className={styles.templateGrid}>{models.map((model) => <article key={model.id}><div className={styles.templateIcon}><Icon name="document" /></div><h3>{model.name}</h3><p>{model.items.length} itens · base de {currency(model.items.reduce((sum, item) => sum + item.unitPrice, 0))}</p><div><button className={styles.secondaryButton} onClick={() => newQuote(model)}>Usar modelo</button><button className={styles.iconButton} aria-label={`Remover modelo ${model.name}`} onClick={() => setModels((current) => current.filter((item) => item.id !== model.id))}><Icon name="trash" /></button></div></article>)}{!models.length ? <EmptyState icon="clipboard" title="Nenhum modelo salvo" description="Crie uma proposta e salve sua estrutura como modelo." /> : null}</div></section> : null}
