...
export function AresApp({ product }: { product: Product }) {
  ...
  function saveDraft(quote: Quote, generated = false) {
    ...
    setSelectedId(normalized.id); setEditing(null); setActive("Orçamentos"); setToast(generated ? "Proposta gerada e marcada como enviada" : "Rascunho salvo");
  }
  ...
  return <AppShell product={product} nav={nav} active={active} onChange={(value) => { setActive(value); if (value === "Criar orçamento" && !editing) newQuote(); }} title={active} subtitle="Propostas editáveis, profissionais e acompanhadas até a decisão." action={<button className={styles.primaryButton} onClick={() => newQuote()}><Icon name="plus" /> Novo orçamento</button>}>
    ...
    {active === "Modelos" ? <section className={styles.pageSheet}><div className={styles.pageHeading}><div><span className={styles.eyebrow}>Modelos reutilizáveis</span><h2>Comece com uma estrutura pronta</h2><p>Reaproveite itens e valores como ponto de partida para novas propostas.</p></div></div><div className={styles.templateGrid}>{models.map((model) => <article key={model.id}><div className={styles.templateIcon}><Icon name="document" /></div><h3>{model.name}</h3><p>{model.items.length} itens · base de {currency(model.items.reduce((sum, item) => sum + item.unitPrice, 0))}</p><div><button className={styles.secondaryButton} onClick={() => newQuote(model)}>Usar modelo</button><button className={styles.iconButton} aria-label={`Remover modelo ${model.name}`} onClick={() => setModels((current) => current.filter((item) => item.id !== model.id))}><Icon name="trash" /></button></div></article>)}</div></section> : null}
