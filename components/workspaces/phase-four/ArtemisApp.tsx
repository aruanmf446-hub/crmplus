...
export function ArtemisApp({ product }: { product: Product }) {
  ...
  function sendKitchen() {
    ...
    const hasReadyTicket = tickets.some((item) => item.tableId === selected.id && item.state === "Pronto");
    setTickets((current) => [...current, ticket]);
    updateTable(selected.id, (table) => ({ ...table, status: hasReadyTicket ? "Pronto" : "Na cozinha", items: table.items.map((item) => pendingItems.some((pending) => pending.id === item.id) ? { ...item, sent: true } : item) }));
