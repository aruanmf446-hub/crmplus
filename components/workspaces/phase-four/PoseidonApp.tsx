...
export function PoseidonApp({ product }: { product: Product }) {
  ...
  const [activityDraft, setActivityDraft] = useState({ date: new Date().toISOString().slice(0, 10), time: "09:00", type: "Ligação", company: "", person: "" });
  ...
  function createOpportunity() {
    ...
    setOpportunities((current) => [next, ...current]); setSelectedId(next.id); setDraft({ company: "", contact: "", phone: "", value: "", owner: "", next: "", heat: "Média" }); setModal(null); setActive("Oportunidades"); setToast("Oportunidade criada");
  }
  ...
  function createActivity() {
    ...
    setActivityDraft({ date: new Date().toISOString().slice(0, 10), time: "09:00", type: "Ligação", company: "", person: "" }); setModal(null); setToast("Atividade agendada");
  }

  function finishActivity() {
    if (!resultDraft.id) return;
    setActivities((current) => current.map((item) => item.id === resultDraft.id ? { ...item, done: true, result: resultDraft.result.trim() || "Concluída" } : item));
    ...
    if (activity) {
      ...
      if (opportunity) setOpportunities((current) => current.map((item) => item.id === opportunity.id ? { ...item, notes: [{ text: `${activity.type}: ${resultDraft.result.trim() || "Concluída"}`, date: todayLabel() }, ...item.notes] } : item));
    }
  ...
  async function addFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (files.some((file) => file.size > 700 * 1024)) {
      setToast("Escolha arquivos menores, com até 700 KB cada");
      event.target.value = "";
      return;
    }
  ...
  return <AppShell product={product} nav={nav} active={active} onChange={setActive} title={active} subtitle="Carteira comercial com contexto, agenda e próxima ação." action={<button className={styles.primaryButton} onClick={() => setModal("new")}><Icon name="plus" /> Nova oportunidade</button>}>
    {active === "Oportunidades" ? <div className={styles.salesLayout}><section className={styles.salesTablePane}><div className={styles.listToolbar}><label className={styles.inputSearch}><Icon name="search" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Empresa, contato ou responsável" /></label><select className={styles.compactSelect} value={stageFilter} onChange={(event) => setStageFilter(event.target.value as Stage | "Todas")}><option>Todas</option><option>Novo contato</option><option>Qualificado</option><option>Proposta enviada</option><option>Negociação</option></select></div><div className={styles.salesHeader}><span>Empresa</span><span>Etapa</span><span>Valor</span><span>Próxima ação</span><span>Responsável</span></div>{filtered.map((item) => <button key={item.id} className={`${styles.salesRow} ${selected?.id === item.id ? styles.salesSelected : ""}`} onClick={() => { setSelectedId(item.id); setDetailTab("Atividade"); }}><div><span className={styles.companyAvatar}>{item.company.slice(0, 2).toUpperCase()}</span><div><strong>{item.company}</strong><small>{item.contact}</small></div></div><StatusPill status={item.stage} /><b>{currency(item.value)}</b><span>{item.next}</span><span>{item.owner}</span></button>)}{!filtered.length ? <EmptyState icon="search" title="Nenhuma oportunidade" description="Altere os filtros ou cadastre uma nova negociação." /> : null}</section>{selected ? <aside className={styles.salesDetail}><div className={styles.detailHeader}><div><span className={styles.eyebrow}>{selected.id}</span><h2>{selected.company}</h2><p>{selected.contact} · {selected.phone}</p></div><StatusPill status={selected.heat} /></div><div className={styles.opportunityValue}><span>Valor estimado</span><strong>{currency(selected.value)}</strong><select value={selected.stage} onChange={(event) => updateSelected({ stage: event.target.value as Stage }, `Etapa alterada para ${event.target.value}`)}><option>Novo contato</option><option>Qualificado</option><option>Proposta enviada</option><option>Negociação</option><option>Ganha</option><option>Perdida</option></select></div><div className={styles.detailTabs}>{["Atividade", "Dados", "Arquivos"].map((tab) => <button key={tab} className={detailTab === tab ? styles.tabActive : ""} onClick={() => setDetailTab(tab)}>{tab}</button>)}</div>{detailTab === "Atividade" ? <><section className={styles.infoSection}><div className={styles.sectionHeading}><div><h3>Próxima ação</h3><p>Mantenha claro o próximo contato necessário.</p></div></div><div className={styles.nextAction}><Icon name="calendar" /><div><strong>{selected.next}</strong><span>Responsável: {selected.owner}</span></div><button onClick={() => updateSelected({ next: "Definir próxima ação" }, "Próxima ação concluída")}>Concluir</button></div></section><section className={styles.infoSection}><div className={styles.sectionHeading}><div><h3>Histórico</h3><p>Contatos e decisões em ordem cronológica.</p></div></div><Timeline items={selected.notes} /></section><div className={styles.quickComposer}><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Registrar nota ou resultado do contato" /><button className={styles.primaryButton} onClick={addNote}>Adicionar registro</button></div></> : null}{detailTab === "Dados" ? <section className={styles.infoSection}><div className={styles.formGrid}><Field label="Empresa"><input value={selected.company} onChange={(event) => updateSelected({ company: event.target.value })} /></Field><Field label="Contato"><input value={selected.contact} onChange={(event) => updateSelected({ contact: event.target.value })} /></Field><Field label="Telefone"><input inputMode="tel" value={selected.phone} onChange={(event) => updateSelected({ phone: event.target.value })} /></Field><Field label="Responsável"><input value={selected.owner} onChange={(event) => updateSelected({ owner: event.target.value })} /></Field><Field label="Valor estimado"><input type="number" value={selected.value} onChange={(event) => updateSelected({ value: Number(event.target.value) || 0 })} /></Field><Field label="Temperatura"><select value={selected.heat} onChange={(event) => updateSelected({ heat: event.target.value as Heat })}><option>Alta</option><option>Média</option><option>Baixa</option></select></Field></div><Field label="Próxima ação"><input value={selected.next} onChange={(event) => updateSelected({ next: event.target.value })} /></Field><div className={styles.closingActions}><button className={styles.secondaryButton} onClick={() => closeOpportunity("Perdida")}>Marcar como perdida</button><button className={styles.primaryButton} onClick={() => closeOpportunity("Ganha")}>Marcar como ganha</button></div></section> : null}{detailTab === "Arquivos" ? <section className={styles.infoSection}><div className={styles.sectionHeading}><div><h3>Arquivos da oportunidade</h3><p>Propostas, imagens e documentos vinculados à negociação.</p></div><label className={styles.secondaryButton}><Icon name="plus" /> Anexar<input hidden type="file" multiple onChange={addFiles} /></label></div><div className={styles.fileRows}>{selected.files.map((file, index) => <div key={`${file.slice(0, 24)}-${index}`}><Icon name="document" /><div><strong>Arquivo {index + 1}</strong><small>Documento vinculado</small></div><button className={styles.iconButton} aria-label={`Remover arquivo ${index + 1}`} onClick={() => updateSelected({ files: selected.files.filter((_, fileIndex) => fileIndex !== index) }, `Arquivo ${index + 1} removido`)}><Icon name="trash" /></button></div>)}</div>{!selected.files.length ? <EmptyState icon="document" title="Sem arquivos anexados" description="Adicione propostas, imagens ou documentos relacionados à oportunidade." action={<label className={styles.primaryButton}>Selecionar arquivos<input hidden type="file" multiple onChange={addFiles} /></label>} /> : null}</section> : null}</aside> : null}</div> : null}
