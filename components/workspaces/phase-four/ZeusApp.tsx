...
const initialVehicles: Vehicle[] = [
  {
    ...
    location: "Obra Norte · informado às 08:20",
    documents: [{ id: "DOC-1", type: "Licenciamento", expiry: "2027-01-31", note: "Documento anual" }],
    maintenance: [{ id: "MAN-1", type: "Troca de óleo e filtros", date: "2026-05-14", odometer: 40220, cost: 780, note: "Preventiva concluída" }],
    fuel: [{ id: "ABS-1", date: "2026-07-20", liters: 52, cost: 338, odometer: 48010, station: "Posto Centro" }],
    history: [{ text: "Quilometragem atualizada para 48.210 km", date: "Hoje, 08:20" }, { text: "Veículo atribuído a Carlos Henrique", date: "20 jul, 15:10" }],
  },
  {
    id: "VEI-013",
    ...
    driverId: "",
    odometer: 72260,
    nextMaintenanceKm: 72000,
    nextMaintenanceDate: "2026-07-19",
    location: "Oficina parceira",
    documents: [{ id: "DOC-2", type: "Seguro", expiry: "2026-08-02", note: "Renovação em análise" }],
    maintenance: [{ id: "MAN-2", type: "Correção do eixo traseiro", date: "2026-07-21", odometer: 72260, cost: 0, note: "Veículo aguardando orçamento" }],
    fuel: [],
    history: [{ text: "Veículo colocado em manutenção", date: "Ontem, 14:30" }, { text: "Desalinhamento traseiro registrado", date: "Ontem, 14:12" }],
  },
  {
    id: "VEI-012",
    ...
    documents: [{ id: "DOC-3", type: "Licenciamento", expiry: "2026-12-20", note: "Regular" }],
    maintenance: [],
    fuel: [{ id: "ABS-2", date: "2026-07-18", liters: 44, cost: 286, odometer: 61210, station: "Posto Avenida" }],
    history: [{ text: "Veículo devolvido ao pátio", date: "18 jul, 18:05" }],
  },
];
...
export function ZeusApp({ product }: { product: Product }) {
  ...
  function createVehicle() {
    const vehicle: Vehicle = {
      ...
      year: Number(vehicleDraft.year) || new Date().getFullYear(),
      status: "Disponível",
      driverId: "",
      odometer: Number(vehicleDraft.odometer) || 0,
      nextMaintenanceKm: (Number(vehicleDraft.odometer) || 0) + 10000,
      nextMaintenanceDate: "",
      location: vehicleDraft.location.trim() || "Local não informado",
      documents: [],
      maintenance: [],
      fuel: [],
      history: [{ text: "Veículo cadastrado", date: todayLabel() }],
    };
    ...
    setModal(null);
    setActive("Frota");
    setToast("Veículo cadastrado localmente");
  }
  ...
  function createDriver() {
    ...
    setDriverDraft({ name: "", phone: "", license: "B", licenseExpiry: "" });
    setModal(null);
    setToast("Motorista cadastrado localmente");
  }
  ...
  function addMaintenance() {
    ...
    const record: MaintenanceRecord = {
      ...
      odometer: Number(maintenanceDraft.odometer) || selected.odometer,
      cost: Number(maintenanceDraft.cost.replace(",", ".")) || 0,
      note: maintenanceDraft.note.trim(),
    };
    updateSelected({
      maintenance: [record, ...selected.maintenance],
      odometer: Math.max(selected.odometer, record.odometer),
      nextMaintenanceKm: Math.max(selected.odometer, record.odometer) + 10000,
      status: selected.status === "Manutenção" ? "Disponível" : selected.status,
    }, `Manutenção registrada: ${record.type}`);
  ...
  function addFuel() {
    ...
    const record: FuelRecord = {
      id: uid("ABS"),
      date: fuelDraft.date,
      liters: Number(fuelDraft.liters.replace(",", ".")) || 0,
      cost: Number(fuelDraft.cost.replace(",", ".")) || 0,
      odometer: Number(fuelDraft.odometer) || selected.odometer,
      station: fuelDraft.station.trim(),
    };
  ...
  function driverName(driverId: string) {
    return drivers.find((driver) => driver.id === driverId)?.name ?? "Sem motorista";
  }

  return <AppShell product={product} nav={nav} active={active} onChange={setActive} title={active} subtitle="Veículos, responsáveis, manutenção, documentos e abastecimentos em registros simples." action={<button className={styles.primaryButton} onClick={() => setModal("vehicle")}><Icon name="plus" /> Novo veículo</button>}>
    {active === "Frota" ? <>
      <div className={zeus.fleetSummary}>
        <div><span>Veículos cadastrados</span><strong>{vehicles.length}</strong></div>
        <div><span>Em uso agora</span><strong>{vehicles.filter((vehicle) => vehicle.status === "Em uso").length}</strong></div>
        <div><span>Precisam de atenção</span><strong>{attentionCount}</strong></div>
      </div>
      <div className={styles.masterDetail}>
        <section className={styles.listPane}>
          <div className={styles.listToolbar}><label className={styles.inputSearch}><Icon name="search" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Placa, modelo ou local" /></label><select className={styles.compactSelect} value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as VehicleStatus | "Todos")}><option>Todos</option><option>Disponível</option><option>Em uso</option><option>Manutenção</option><option>Inativo</option></select></div>
          <div className={styles.recordList}>{filteredVehicles.map((vehicle) => <button key={vehicle.id} className={`${styles.recordRow} ${selected?.id === vehicle.id ? styles.recordSelected : ""}`} onClick={() => { setSelectedId(vehicle.id); setDetailTab("Resumo"); }}><div className={styles.recordAvatar}><Icon name="car" /></div><div className={styles.recordMain}><div><strong>{vehicle.model}</strong><span>{vehicle.plate}</span></div><p>{driverName(vehicle.driverId)} · {vehicle.odometer.toLocaleString("pt-BR")} km</p></div><div className={styles.recordMeta}><StatusPill status={vehicle.status} /><small>{needsAttention(vehicle) ? "Atenção necessária" : vehicle.location}</small></div></button>)}{!filteredVehicles.length ? <EmptyState icon="search" title="Nenhum veículo encontrado" description="Altere a busca ou cadastre um novo veículo." /> : null}</div>
        </section>
        {selected ? <section className={styles.detailPane}>
          <div className={styles.detailHeader}><div className={zeus.vehicleIdentity}><div className={zeus.vehicleIcon}><Icon name="car" /></div><div><span className={styles.eyebrow}>{selected.id}</span><h2>{selected.model}</h2><p>{selected.plate} · {selected.year}</p></div></div><div className={styles.headerButtons}><StatusPill status={selected.status} /><button className={styles.secondaryButton} onClick={() => setModal("maintenance")}>Registrar manutenção</button></div></div>
          <div className={styles.detailTabs}>{["Resumo", "Manutenção", "Documentos", "Combustível", "Histórico"].map((tab) => <button key={tab} className={detailTab === tab ? styles.tabActive : ""} onClick={() => setDetailTab(tab)}>{tab}</button>)}</div>
          <div className={styles.detailBody}>
            {detailTab === "Resumo" ? <>
              <div className={styles.summaryGrid}><div><span>Situação</span><StatusPill status={selected.status} /></div><div><span>Motorista</span><strong>{driverName(selected.driverId)}</strong></div><div><span>Quilometragem</span><strong>{selected.odometer.toLocaleString("pt-BR")} km</strong></div><div><span>Local informado</span><strong>{selected.location}</strong></div></div>
              {needsAttention(selected) ? <div className={zeus.attentionBox}><Icon name="warning" /><div><strong>Este veículo precisa de atenção</strong><span>{maintenanceAlert(selected)}</span></div></div> : null}
              <section className={styles.infoSection}><div className={styles.sectionHeading}><div><h3>Dados operacionais</h3><p>Alterações são salvas automaticamente neste navegador.</p></div></div><div className={zeus.detailFields}><label><span>Situação</span><select value={selected.status} onChange={(event) => updateSelected({ status: event.target.value as VehicleStatus }, `Situação alterada para ${event.target.value}`)}><option>Disponível</option><option>Em uso</option><option>Manutenção</option><option>Inativo</option></select></label><label><span>Motorista responsável</span><select value={selected.driverId} onChange={(event) => updateSelected({ driverId: event.target.value }, `Motorista alterado para ${driverName(event.target.value)}`)}><option value="">Sem motorista</option>{drivers.map((driver) => <option key={driver.id} value={driver.id}>{driver.name}</option>)}</select></label><label><span>Quilometragem atual</span><input type="number" value={selected.odometer} onChange={(event) => updateSelected({ odometer: Number(event.target.value) || 0 })} /></label><label><span>Local informado</span><input value={selected.location} onChange={(event) => updateSelected({ location: event.target.value })} /></label><label><span>Próxima manutenção em km</span><input type="number" value={selected.nextMaintenanceKm} onChange={(event) => updateSelected({ nextMaintenanceKm: Number(event.target.value) || 0 })} /></label><label><span>Próxima manutenção por data</span><input type="date" value={selected.nextMaintenanceDate} onChange={(event) => updateSelected({ nextMaintenanceDate: event.target.value })} /></label></div></section>
            </> : null}
            {detailTab === "Manutenção" ? <section className={styles.infoSection}><div className={styles.sectionHeading}><div><h3>Histórico de manutenção</h3><p>Serviços preventivos e corretivos registrados.</p></div><button onClick={() => setModal("maintenance")}><Icon name="plus" /> Adicionar</button></div><div className={zeus.simpleRows}>{selected.maintenance.map((record) => <div key={record.id}><div><strong>{record.type}</strong><p>{formatDate(record.date)} · {record.odometer.toLocaleString("pt-BR")} km</p><small>{record.note || "Sem observação"}</small></div><span className={zeus.rowValue}>{currency(record.cost)}</span><button className={styles.iconButton} onClick={() => updateSelected({ maintenance: selected.maintenance.filter((item) => item.id !== record.id) }, `${record.type} removida do histórico`)}><Icon name="trash" /></button></div>)}{!selected.maintenance.length ? <EmptyState icon="settings" title="Sem manutenção registrada" description="Adicione o primeiro serviço deste veículo." /> : null}</div></section> : null}
            {detailTab === "Documentos" ? <section className={styles.infoSection}><div className={styles.sectionHeading}><div><h3>Documentos e vencimentos</h3><p>Controle manual, sem consulta a órgãos externos.</p></div><button onClick={() => setModal("document")}><Icon name="plus" /> Adicionar</button></div><div className={zeus.simpleRows}>{selected.documents.map((document) => <div key={document.id}><div><strong>{document.type}</strong><p>Vencimento: {formatDate(document.expiry)}</p><small>{document.note || "Sem observação"}</small></div><StatusPill status={documentStatus(document.expiry)} /><button className={styles.iconButton} onClick={() => updateSelected({ documents: selected.documents.filter((item) => item.id !== document.id) }, `${document.type} removido`)}><Icon name="trash" /></button></div>)}{!selected.documents.length ? <EmptyState icon="document" title="Nenhum documento cadastrado" description="Registre licenciamento, seguro ou outro vencimento." /> : null}</div></section> : null}
            {detailTab === "Combustível" ? <section className={styles.infoSection}><div className={styles.sectionHeading}><div><h3>Abastecimentos</h3><p>Registro simples de litros, valor e quilometragem.</p></div><button onClick={() => setModal("fuel")}><Icon name="plus" /> Registrar</button></div><div className={zeus.simpleRows}>{selected.fuel.map((record) => <div key={record.id}><div><strong>{record.liters.toLocaleString("pt-BR")} L · {record.station || "Posto não informado"}</strong><p>{formatDate(record.date)} · {record.odometer.toLocaleString("pt-BR")} km</p></div><span className={zeus.rowValue}>{currency(record.cost)}</span><button className={styles.iconButton} onClick={() => updateSelected({ fuel: selected.fuel.filter((item) => item.id !== record.id) }, "Abastecimento removido")}><Icon name="trash" /></button></div>)}{!selected.fuel.length ? <EmptyState icon="activity" title="Sem abastecimentos" description="Registre o primeiro abastecimento deste veículo." /> : null}</div></section> : null}
    ...
    {active === "Motoristas" ? <section className={styles.pageSheet}><div className={styles.pageHeading}><div><span className={styles.eyebrow}>Condutores</span><h2>Motoristas cadastrados</h2><p>Responsável, categoria da habilitação e validade.</p></div><button className={styles.primaryButton} onClick={() => setModal("driver")}><Icon name="plus" /> Novo motorista</button></div><div className={zeus.simpleRows}>{drivers.map((driver) => <div className={zeus.driverCard} key={driver.id}><span className={zeus.driverAvatar}>{driver.name.slice(0, 2).toUpperCase()}</span><div><h3>{driver.name}</h3><p>CNH {driver.license} · validade {formatDate(driver.licenseExpiry)} · {vehicles.filter((vehicle) => vehicle.driverId === driver.id).length} veículo(s)</p></div><div className={zeus.inlineActions}><StatusPill status={driver.status} /><button className={styles.iconButton} onClick={() => { setDrivers((current) => current.filter((item) => item.id !== driver.id)); setVehicles((current) => current.map((vehicle) => vehicle.driverId === driver.id ? { ...vehicle, driverId: "" } : vehicle)); }}><Icon name="trash" /></button></div></div>)}</div></section> : null}

    {active === "Documentos" ? <section className={styles.pageSheet}><div className={styles.pageHeading}><div><span className={styles.eyebrow}>Vencimentos</span><h2>Documentos da frota</h2><p>Lista única para encontrar pendências sem abrir veículo por veículo.</p></div></div><div className={zeus.simpleRows}>{vehicles.flatMap((vehicle) => vehicle.documents.map((document) => ({ vehicle, document }))).sort((a, b) => a.document.expiry.localeCompare(b.document.expiry)).map(({ vehicle, document }) => <button key={document.id} onClick={() => openVehicle(vehicle.id, "Documentos")}><div><strong>{document.type} · {vehicle.plate}</strong><p>{vehicle.model} · vence em {formatDate(document.expiry)}</p></div><StatusPill status={documentStatus(document.expiry)} /><Icon name="chevron" /></button>)}</div></section> : null}

    {active === "Combustível" ? <section className={styles.pageSheet}><div className={styles.pageHeading}><div><span className={styles.eyebrow}>Registros locais</span><h2>Abastecimentos da frota</h2><p>Valores informados manualmente, sem integração com cartões ou postos.</p></div></div><div className={zeus.fuelTotals}><div><span>Total registrado</span><strong>{currency(totalFuelCost)}</strong></div><div><span>Volume registrado</span><strong>{totalFuelLiters.toLocaleString("pt-BR")} L</strong></div></div><div className={zeus.simpleRows}>{vehicles.flatMap((vehicle) => vehicle.fuel.map((record) => ({ vehicle, record }))).sort((a, b) => b.record.date.localeCompare(a.record.date)).map(({ vehicle, record }) => <button key={record.id} onClick={() => openVehicle(vehicle.id, "Combustível")}><div><strong>{vehicle.plate} · {record.liters.toLocaleString("pt-BR")} L</strong><p>{formatDate(record.date)} · {record.station || "Posto não informado"}</p></div><span className={zeus.rowValue}>{currency(record.cost)}</span><Icon name="chevron" /></button>)}</div></section> : null}

    <Modal open={modal === "vehicle"} title="Novo veículo" description="Cadastre somente os dados necessários para começar." onClose={() => setModal(null)}><Form onSubmit={createVehicle}><div className={styles.formGrid}><Field label="Modelo"><input required value={vehicleDraft.model} onChange={(event) => setVehicleDraft((current) => ({ ...current, model: event.target.value }))} /></Field><Field label="Placa"><input required value={vehicleDraft.plate} onChange={(event) => setVehicleDraft((current) => ({ ...current, plate: event.target.value.toUpperCase() }))} /></Field><Field label="Ano"><input type="number" value={vehicleDraft.year} onChange={(event) => setVehicleDraft((current) => ({ ...current, year: event.target.value }))} /></Field><Field label="Quilometragem"><input type="number" value={vehicleDraft.odometer} onChange={(event) => setVehicleDraft((current) => ({ ...current, odometer: event.target.value }))} /></Field></div><Field label="Local inicial"><input value={vehicleDraft.location} onChange={(event) => setVehicleDraft((current) => ({ ...current, location: event.target.value }))} /></Field><div className={styles.modalActions}><button className={styles.primaryButton}>Cadastrar veículo</button></div></Form></Modal>

    <Modal open={modal === "driver"} title="Novo motorista" onClose={() => setModal(null)}><Form onSubmit={createDriver}><Field label="Nome"><input required value={driverDraft.name} onChange={(event) => setDriverDraft((current) => ({ ...current, name: event.target.value }))} /></Field><div className={styles.formGrid}><Field label="Telefone"><input value={driverDraft.phone} onChange={(event) => setDriverDraft((current) => ({ ...current, phone: event.target.value }))} /></Field><Field label="Categoria da CNH"><input value={driverDraft.license} onChange={(event) => setDriverDraft((current) => ({ ...current, license: event.target.value }))} /></Field></div><Field label="Validade da CNH"><input type="date" value={driverDraft.licenseExpiry} onChange={(event) => setDriverDraft((current) => ({ ...current, licenseExpiry: event.target.value }))} /></Field><div className={styles.modalActions}><button className={styles.primaryButton}>Cadastrar motorista</button></div></Form></Modal>

    <Modal open={modal === "maintenance"} title="Registrar manutenção" description={selected ? `${selected.model} · ${selected.plate}` : undefined} onClose={() => setModal(null)}><Form onSubmit={addMaintenance}><Field label="Serviço"><input required value={maintenanceDraft.type} onChange={(event) => setMaintenanceDraft((current) => ({ ...current, type: event.target.value }))} /></Field><div className={styles.formGrid}><Field label="Data"><input type="date" value={maintenanceDraft.date} onChange={(event) => setMaintenanceDraft((current) => ({ ...current, date: event.target.value }))} /></Field><Field label="Quilometragem"><input type="number" value={maintenanceDraft.odometer} onChange={(event) => setMaintenanceDraft((current) => ({ ...current, odometer: event.target.value }))} placeholder={selected ? String(selected.odometer) : ""} /></Field><Field label="Custo"><input inputMode="decimal" value={maintenanceDraft.cost} onChange={(event) => setMaintenanceDraft((current) => ({ ...current, cost: event.target.value }))} /></Field></div><Field label="Observação"><textarea value={maintenanceDraft.note} onChange={(event) => setMaintenanceDraft((current) => ({ ...current, note: event.target.value }))} /></Field><div className={styles.modalActions}><button className={styles.primaryButton}>Salvar manutenção</button></div></Form></Modal>
