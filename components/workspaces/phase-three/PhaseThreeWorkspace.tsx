...
function DistributionPage() { return <section className={styles.pageSheet}><div className={styles.pageHeading}><div><span className={styles.eyebrow}>Compartilhamento</span><h2>Links e canais</h2><p>Distribua a pesquisa pelos canais usados pela empresa.</p></div></div><div className={styles.linkRows}><div><Icon name="message" /><div><strong>WhatsApp</strong><span>Link curto pronto para envio</span></div><button>Copiar link</button></div><div><Icon name="message" /><div><strong>E-mail</strong><span>Convite com texto personalizado</span></div><button>Preparar envio</button></div><div><Icon name="document" /><div><strong>QR Code</strong><span>Material para impressão no atendimento</span></div><button>Baixar</button></div></div></section>; }
