      ...
      <main>
        <section className="detail-hero shell">
          ...
          <div className="detail-hero-grid">
            ...
            <div className="workflow-panel" aria-label={`Fluxo principal do ${product.name}`}><p>Uma rotina clara, do início ao fim</p><ol>{product.workflow.map((step, index) => <li key={step}><span>{String(index + 1).padStart(2, "0")}</span><b>{step}</b></li>)}</ol></div>
          </div>
        </section>
        ...
        <section className="features shell" id="recursos"><div className="section-intro compact"><p className="eyebrow">Recursos essenciais</p><h2>Somente o que ajuda a rotina a avançar.</h2></div><div className="feature-list">{product.features.map((feature, index) => <div key={feature}><span>{String(index + 1).padStart(2, "0")}</span><h3>{feature}</h3><p>Organizado para ser entendido e usado desde o primeiro dia.</p></div>)}</div></section>
        <section className="product-next shell"><div><p className="eyebrow">Veja funcionando</p><h2>Explore a estrutura visual do {product.shortName} com dados simulados.</h2></div><Link href={`/sistemas/${product.slug}`}>Abrir demonstração <span aria-hidden="true">→</span></Link></section>
      </main>
      <Footer />
    </div>
  );
